...
### Fixed
- **`Retrieve` is capped + pageable — closing the footgun behind the ACP context overflow.** `Retrieve(ref)` returned a sealed blob's *entire* content unbounded, so a single call could pull a 1.3 MB blob (≈340k tokens) into context and blow past the model's window in one turn (the trigger for the 0.29.0 compaction-ceiling bug). It now returns at most `KORGEX_RETRIEVE_MAX_CHARS` (default 100,000) chars; when the blob is larger the result is `{truncated: true, next_offset: N}` and the model pages the rest with `Retrieve(ref, offset=N)`. Small blobs are unchanged (returned whole); the full original stays content-addressed + verifiable in the ledger. Pairs with the 0.29.0 compaction fix: that *contains* an over-ceiling transcript, this *prevents* one Retrieve from causing it.
...
### Added
- **MCP catalog: `context7` preset (`korgex mcp add context7`).** [Context7](https://context7.com) (Upstash) serves up-to-date documentation and code examples for any library/framework over a hosted MCP endpoint. One-command preset wires it in (anonymous; an API key raises limits) so the agent can pull current, version-correct docs into context instead of relying on stale training data.
...
### Added
- **ACP: mid-turn `session/cancel` — an editor's "stop" now actually interrupts a turn.** Previously the cancel flag was only read *after* the turn finished: a single-threaded stdio loop can't read the cancel notification while a turn is executing. `serve()` now reads stdin on a **background thread** — a `session/cancel` flips the session's cancel flag the instant it arrives (handled inline, off the queue), and the agent loop checks a `_should_cancel` callback **between rounds** and stops cleanly (`stopReason: cancelled`). The blocking permission round-trip (`session/request_permission`) consumes from the same queue, so it still works alongside the reader thread. Cooperative (takes effect at the next round boundary, not mid-network-call); covered by an agent-stop test, the bridge wiring, and a threaded end-to-end transport test.

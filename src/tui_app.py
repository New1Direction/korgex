"""Bottom-pinned inline TUI for korgex (Hermes-style architecture).
...
def run_app(repl) -> None:
    ...
    # Push the whole TUI to the bottom of the terminal (Hermes's trick).
    try:
    ...
    # Paint the wordmark + welcome panel (with the static gradient portal in it)
    # ABOVE the app, where it stays in scrollback. (An in-app animated portal
    # fought this static banner — different coordinate space — so we keep the
    # banner as the clean, cohesive splash.)
    repl._banner()

...
def estimate_cost(events) -> dict:
    ...
    for e in events or []:
        ...
        pin_tok = _int(args.get("prompt_tokens"))
        ...
        usd = pin_tok / 1_000_000 * in_rate + pout_tok / 1_000_000 * out_rate
        slot["usd"] += usd
        slot["known"] = True
        total_usd += usd
    return {
...
def format_cost(summary: dict) -> str:
    """One-line estimate: ``≈ $0.0123 (est.) · 12.3k in / 4.5k out``. Notes unknown
    models so the figure is never silently incomplete."""
    s = summary or {}

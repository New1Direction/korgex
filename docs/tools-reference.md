...
- **Internal handlers** (49+) — what actually runs. Jules-style `tool_*` functions in `src/tools_impl.py`. The router translates user-facing calls into internal handler calls.
...
| `tool_google_search` | Web search (requires `hermes_tools`). |
